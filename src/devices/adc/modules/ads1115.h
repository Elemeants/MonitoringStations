#if !defined(ADC_MODULES_ADS1115_H)
#define ADC_MODULES_ADS1115_H
...
#define ADS1115_IIC_ADDRESS2 (0x4A)
#define ADS1115_IIC_ADDRESS3 (0x4B)
...
#define ADS1115_OS_MASK \
    (0x0001)                          // Operational status/single-shot conversion start
...
// Diferential modes of the adc
typedef enum
{
    // Differential measure from CH0 to CH1
    ADC_DIFF_CH0_CH1 = 01,
    // Differential measure from CH0 to CH3
    ADC_DIFF_CH0_CH3 = 03,
    // Differential measure from CH1 to CH3
    ADC_DIFF_CH1_CH3 = 13,
    // Differential measure from CH2 to CH3
    ADC_DIFF_CH2_CH3 = 23
} eAdcDifferentialMode_t;

class ADS1115 : public IAdcI2CDevice
{
protected:
...
public:
    ...
    void init(void);
    bool checkADS1115();
    void setCompQue(eADSCompQue_t value), setCompLat(eADSCompLat_t value),
        setCompPol(eADSCompPol_t value), setCompMode(eADSCompMode_t value),
        setRate(eADSRate_t value), setMode(eADSMode_t value),
        setGain(eADSGain_t value), setMux(eADSMux_t value),
    ...
    uint16_t read(uint8_t channel) { return readVoltage(channel); }
    uint16_t readDiff(uint8_t mode) { return ComparatorVoltage(mode); };
    uint16_t readDiff(eAdcDifferentialMode_t mode)
    {
        return readDiff((uint8_t)mode);
    }

    bool begin(uint8_t addr)
    {
        setAddr_ADS1115(addr);
        setGain(eGAIN_TWOTHIRDS);
        setMode(eMODE_SINGLE);
        setRate(eRATE_128);
        setOSMode(eOSMODE_SINGLE);
        return IAdcI2CDevice::begin(addr);
    }
};

#endif  // ADC_MODULES_ADS1115_H
