...
void ADS1115::setLowThreshold(int16_t threshold)
{
    ...
    writeAdsReg(ads_i2cAddress, ADS1115_POINTER_LOWTHRESH, ads_lowthreshold);
}

/*
 *Gets the low threshold value
 */
int16_t ADS1115::getLowThreshold() { return ads_lowthreshold; }
...
void ADS1115::setHighThreshold(int16_t threshold)
{
    ...
    writeAdsReg(ads_i2cAddress, ADS1115_POINTER_HITHRESH, ads_highthreshold);
}

/*
 *Gets the high threshold value
 */
int16_t ADS1115::getHighThreshold() { return ads_highthreshold; }

void ADS1115::setCompQue(eADSCompQue_t value)
{
    int16_t conf;
    conf = readAdsReg(ads_i2cAddress, ADS1115_POINTER_CONFIG);
    conf &= ~((uint16_t)0x03);
    conf |= (uint16_t)value;
    writeAdsReg(ads_i2cAddress, ADS1115_POINTER_CONFIG, conf);
}
...
uint16_t ADS1115::readVoltage(uint8_t channel)
{
    ...
    readAdsReg(ads_i2cAddress, ADS1115_POINTER_CONFIG);
    delay(ads_conversionDelay);
    // Read the conversion results
    // 16-bit unsigned results for the ADS1115
    int16_t Voltage = 0;
    Voltage = (int16_t)readAdsReg(ads_i2cAddress, ADS1115_POINTER_CONVERT) *
              coefficient;
    return Voltage;
}
/*
...
void ADS1115::writeReg(uint8_t i2cAddress, uint8_t reg, uint8_t *pBuf,
                       uint16_t len)
{
    port->begin();
    port->beginTransmission(i2cAddress);
    port->write(reg);
    for (uint16_t i = 0; i < len; i++) port->write(pBuf[i]);
    port->endTransmission();
}

void ADS1115::readReg(uint8_t i2cAddress, uint8_t reg, uint8_t *pBuf,
                      uint16_t len)
{
    port->begin();
    port->beginTransmission(i2cAddress);
    ...
    if (port->endTransmission() != 0)
        return;
    port->requestFrom(i2cAddress, (uint8_t)len);
