...
#include "common.h"
#include "registers/json_measure_register.h"

// #define GPRS_SW_TX 25
// #define GPRS_SW_RX 26
#define GPRS_RST 32
...
#define GPRS_POST_HOST "datospruebas.agricos.mx:80/api/Measures"

static Adafruit_FONA gsm_device = Adafruit_FONA(GPRS_RST);
...
static char body_post_buffer[GSM_GPRS_HTTP_BUFFER_SIZE];
...
void GSM_Init()
{
    ...
    GSM_PORT.begin(GSM_SERIAL_BAUDRATE);
    ...
    while (!isBegined && attempts < 3)
    {
        ...
        isBegined = gsm_device.begin(GSM_PORT);
        if (isBegined)
        {
