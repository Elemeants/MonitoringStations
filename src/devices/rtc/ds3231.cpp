...
bool DS3231::begin(uint8_t addr)
{
    setAddress(addr);
    bool connected = I2CDevice::begin(addr);
...
{
    logger << LOG_DEBUG << F("    ") << year << " " << month << " " << day << " " << hour << " " << minute << " " << second << EndLine;
    KernelWire.beginTransmission(DS3231_ADDRESS);

    #if ARDUINO >= 100
        KernelWire.write(DS3231_REG_TIME);
    #else
        KernelWire.send(DS3231_REG_TIME);
    #endif

    #if ARDUINO >= 100
        KernelWire.write(dec2bcd(second));
        KernelWire.write(dec2bcd(minute));
        KernelWire.write(dec2bcd(hour));
        KernelWire.write(dec2bcd(dow(year, month, day)));
        KernelWire.write(dec2bcd(day));
        KernelWire.write(dec2bcd(month));
        KernelWire.write(dec2bcd(year-2000));
    #else
        KernelWire.send(dec2bcd(second));
        KernelWire.send(dec2bcd(minute));
        KernelWire.send(dec2bcd(hour));
        KernelWire.send(dec2bcd(dow(year, month, day)));
        KernelWire.send(dec2bcd(day));
        KernelWire.send(dec2bcd(month));
        KernelWire.send(dec2bcd(year-2000));
    #endif
...
void DS3231::setDateTime(const char* date, const char* time)
{
    ...
    logger << LOG_DEBUG << F("    ") << date << " " << time << EndLine;
...
{
    // logger << LOG_DEBUG << F("      getDateTime: called") << EndLine;
    int values[7];
    ...
    // logger << LOG_DEBUG << F("      getDateTime: requesting from") << EndLine;
    uint8_t bytes = KernelWire.requestFrom(DS3231_ADDRESS, 7);

    if (bytes != 7) {
        // logger << LOG_ERROR << F("      getDateTime: rtc not responded") << EndLine;
        return t;
    }

    while(!KernelWire.available()) { };

    // logger << LOG_DEBUG << F("      getDateTime: parsing response -> ");
    for (int i = 6; i >= 0; i--)
    {
        uint8_t byte = KernelWire.read();
        // logger << F(" 0x") << String(byte, HEX);
        #if ARDUINO >= 100
            values[i] = bcd2dec(byte);
        #else
            values[i] = bcd2dec(KernelWire.receive());
        #endif
    }
    // logger << EndLine;
    ...
    // logger << LOG_DEBUG << F("      getDateTime: assing response") << EndLine;
    ...
    // logger << LOG_DEBUG << F("      getDateTime: ") <<
    //     t.year << F("-") << t.month << F("-") << t.day << " " << t.hour << F(":") << t.minute << F(":") << t.second << EndLine;
    return t;
}
...
float DS3231::readTemperature(void)
{
    ...
    KernelWire.beginTransmission(DS3231_ADDRESS);
    #if ARDUINO >= 100
        KernelWire.write(DS3231_REG_TEMPERATURE);
    #else
        KernelWire.send(DS3231_REG_TEMPERATURE);
    #endif
    KernelWire.endTransmission();

    KernelWire.requestFrom(DS3231_ADDRESS, 2);
    ...
    lsb = KernelWire.read();
    #else
    msb = KernelWire.receive();
    lsb = KernelWire.receive();
    #endif
...
RTCAlarmTime DS3231::getAlarm1(void)
{
    ...
    KernelWire.beginTransmission(DS3231_ADDRESS);
    #if ARDUINO >= 100
        KernelWire.write(DS3231_REG_ALARM_1);
    #else
        KernelWire.send(DS3231_REG_ALARM_1);
    #endif
    KernelWire.endTransmission();

    KernelWire.requestFrom(DS3231_ADDRESS, 4);
    ...
    {
        #if ARDUINO >= 100
            values[i] = bcd2dec(KernelWire.read() & 0b01111111);
        #else
            values[i] = bcd2dec(KernelWire.receive() & 0b01111111);
        #endif
    }
...
DS3231_alarm1_t DS3231::getAlarmType1(void)
{
    ...
    {
        #if ARDUINO >= 100
            values[i] = bcd2dec(KernelWire.read());
        #else
            values[i] = bcd2dec(KernelWire.receive());
        #endif
    }
...
void DS3231::setAlarm1(uint8_t dydw, uint8_t hour, uint8_t minute, uint8_t second, DS3231_alarm1_t mode, bool armed)
{
    ...
    #if ARDUINO >= 100
        KernelWire.write(DS3231_REG_ALARM_1);
        KernelWire.write(second);
        KernelWire.write(minute);
        KernelWire.write(hour);
        KernelWire.write(dydw);
    #else
        KernelWire.send(DS3231_REG_ALARM_1);
        KernelWire.send(second);
        KernelWire.send(minute);
        KernelWire.send(hour);
        KernelWire.send(dydw);
    #endif
...
RTCAlarmTime DS3231::getAlarm2(void)
{
    ...
    KernelWire.beginTransmission(DS3231_ADDRESS);
    #if ARDUINO >= 100
        KernelWire.write(DS3231_REG_ALARM_2);
    #else
        KernelWire.send(DS3231_REG_ALARM_2);
    #endif
    KernelWire.endTransmission();

    KernelWire.requestFrom(DS3231_ADDRESS, 3);
...
void DS3231::writeRegister8(uint8_t reg, uint8_t value)
{
    KernelWire.beginTransmission(DS3231_ADDRESS);
    #if ARDUINO >= 100
        KernelWire.write(reg);
        KernelWire.write(value);
    #else
        KernelWire.send(reg);
        KernelWire.send(value);
    #endif
    KernelWire.endTransmission();
}
...
uint8_t DS3231::readRegister8(uint8_t reg)
{
    ...
    KernelWire.requestFrom(DS3231_ADDRESS, 1);
    while(!KernelWire.available()) {};
    #if ARDUINO >= 100
        value = KernelWire.read();
    #else
        value = KernelWire.receive();
    #endif
    KernelWire.endTransmission();
